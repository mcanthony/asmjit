...
struct Context {
  ...
  //! Get stop node.
  ASMJIT_INLINE ASNode* getStop() const { return _stop; }

  //! Get start of the current scope.
  ASMJIT_INLINE ASNode* getStart() const { return _start; }
  //! Get end of the current scope.
  ASMJIT_INLINE ASNode* getEnd() const { return _end; }

  //! Get extra block.
  ASMJIT_INLINE ASNode* getExtraBlock() const { return _extraBlock; }
  //! Set extra block.
  ASMJIT_INLINE void setExtraBlock(ASNode* node) { _extraBlock = node; }
  ...
  ASMJIT_INLINE BitArray* newBits(uint32_t len) {
    return static_cast<BitArray*>(
      _baseZone.allocZeroed(static_cast<size_t>(len) * BitArray::kEntitySize));
  }

  ASMJIT_INLINE BitArray* copyBits(const BitArray* src, uint32_t len) {
    return static_cast<BitArray*>(
      _baseZone.dup(src, static_cast<size_t>(len) * BitArray::kEntitySize));
  }
  ...
  ASMJIT_INLINE Error addUnreachableNode(ASNode* node) {
    PodList<ASNode*>::Link* link = _baseZone.allocT<PodList<ASNode*>::Link>();
    if (link == NULL)
  ...
  //! should start).
  ASMJIT_INLINE Error addReturningNode(ASNode* node) {
    PodList<ASNode*>::Link* link = _baseZone.allocT<PodList<ASNode*>::Link>();
    if (link == NULL)
      return setError(kErrorNoHeapMemory);
  ...
  ASMJIT_INLINE Error addJccNode(ASNode* node) {
    PodList<ASNode*>::Link* link = _baseZone.allocT<PodList<ASNode*>::Link>();
    if (link == NULL)
      return setError(kErrorNoHeapMemory);
  ...
  virtual Error compile(ASFunc* func);

  // --------------------------------------------------------------------------
  // [Serialize]
  // --------------------------------------------------------------------------

  virtual Error serialize(Assembler* assembler, ASNode* start, ASNode* stop) = 0;
  ...
  //! Unreachable nodes.
  PodList<ASNode*> _unreachableList;
  //! Returning nodes.
  PodList<ASNode*> _returningList;
  //! Jump nodes.
  PodList<ASNode*> _jccList;
