...
ASMJIT_ENUM(CallConv) {
  ...
  //! \internal
  _kCallConvX86Start = kCallConvX86CDecl,
  //! \internal
  _kCallConvX86End = kCallConvX86GccRegParm3,

  //! \internal
  _kCallConvX64Start = kCallConvX64Win,
  //! \internal
  _kCallConvX64End = kCallConvX64Unix,
...
#else
# error "[asmjit] Couldn't determine a target calling convention."
#endif
};
...
ASMJIT_ENUM(FuncHint) {
  ...
  //! prolog and epilog. In addition, on X86/64 it will save one register (ebp
  //! or rbp), which can be used by the function instead.
  kFuncHintNaked = 0,
